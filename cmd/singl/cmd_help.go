...
Verbs:
  ...
  wait-all   [--timeout SECS] [--interval SECS]           block until all active agents settle
...
Verbs:
  list      --project <handle>                                   persisted workflows
  create    --project <handle> --branch <name> [--base-dir D]    one worktree per repo
  remove    --project <handle> --branch <name>                   tear the workflow down
  discover  --project <handle>                                   (streaming)
`,
...
Verbs:
  ...
  merge     --repo P --branch <name>   merge a branch into the repo's default branch
`,
...
Verbs:
  ...
  provider  [--repo P]   provider for one repo's origin remote (github/gitlab/gitea)
`,
