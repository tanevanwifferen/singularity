...
- `Flow.Start` returns `BAD_REQUEST` for an empty goal, a `work_dir` that is not an existing directory, a `max_rounds` outside 1..20, and for `opts.use_worktree`/`review_opts.use_worktree`/`plan_opts.use_worktree` — which a flow refuses rather than silently clears, because every round must see the same tree. `Flow.Remove` returns `CONFLICT` while the flow is non-terminal. `FlowStartRequest.enable_planning` (default `false`) submits an optional planning task, on `plan_opts` (defaulting to `opts`, the same rule `review_opts` follows), before round 1 — see §12 of `docs/design/ADVERSARIAL-REVIEW.md`.
