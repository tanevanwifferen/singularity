...
// Gitea. Sources, in order: gh CLI, the glab CLI config file (per host), the
// GITHUB_TOKEN / GITLAB_TOKEN environment variables, and finally tea, so an
// existing gh/glab setup keeps winning exactly as it did before Gitea
// support was added.
...
// we deliberately never read it. The note return describes why detection
// failed (for the Detail aggregation).
func detectGiteaAuth() (*ForgeAuth, string) {
	if !hasForgeCLI(teaBin) {
		return nil, "tea CLI (not installed)"
	}
