...
func NewHelpOverlay(bindings []HelpBinding) HelpOverlay {
	return HelpOverlay{
		Modal: Modal{
...
func (h HelpOverlay) Update(msg tea.Msg) (HelpOverlay, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		h.Modal.SetSize(msg.Width, msg.Height)
		// Adjust visible count based on terminal height
		h.visible = msg.Height - 10
		if h.visible < 5 {
			h.visible = 5
		}
	}
...
func (h HelpOverlay) View(background string) string {
	...
	h.Modal.Content = strings.Join(rows, "\n")
	return h.Modal.Render(background)
}
