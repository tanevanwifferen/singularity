...
func (v *StubView1) View() string {
	theme := GetTheme()
	s := theme.DashboardTitle.Render(" Stub View 1 ")
	s += "\n\n"
	s += theme.StatsStyle.Render(fmt.Sprintf(" Repository: %s", v.repoPath))
	s += "\n\n"
	s += theme.InfoStyle.Render(" This is Stub View 1 - used for testing the router.")
	s += "\n\n"
	s += theme.Help.Render(" Press 2 to switch to Stub View 2")
	s += "\n"
	s += theme.Help.Render(" Press 1 to stay on Stub View 1")
	return s
}
