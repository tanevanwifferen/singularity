...
func (v *OverviewView) View() string {
	...
	// Header
	s.WriteString(th.DashboardTitle.Render(" Repository Overview "))
	s.WriteString("\n\n")
	...
	if v.repo != nil {
		...
		s.WriteString(fmt.Sprintf(" %s %s", th.BranchStyle.Render("Branch:"), branchDisplay))
		...
		s.WriteString(fmt.Sprintf(" %s %s\n", th.BranchStyle.Render("HEAD:"), th.InfoStyle.Render(headShort)))
		...
		if v.repo.CurrentBranch != "" {
			...
			if branch != nil && branch.Upstream != "" {
				s.WriteString(fmt.Sprintf(" %s %s → %s",
					th.BranchStyle.Render("Sync:"),
					th.StatsStyle.Render(v.repo.CurrentBranch),
					th.StatsStyle.Render(branch.Upstream)))

				if branch.Ahead > 0 || branch.Behind > 0 {
					if branch.Ahead > 0 {
						s.WriteString(th.DashboardAccentStyle.Render(fmt.Sprintf(" ↑%d", branch.Ahead)))
					}
					if branch.Behind > 0 {
						s.WriteString(th.DashboardErrorStyle.Render(fmt.Sprintf(" ↓%d", branch.Behind)))
					}
				} else {
					s.WriteString(th.StatsStyle.Render(" (up to date)"))
				}
				s.WriteString("\n")
			}
		}
		...
		s.WriteString(fmt.Sprintf(" %s %s   %s %s\n",
			...
			th.StatsStyle.Render(fmt.Sprintf("%d", v.worktreeCnt))))
	}

	s.WriteString("\n")

	// Recent commits section
	s.WriteString(th.StatsStyle.Render(" Recent Commits "))
	s.WriteString("\n")
	s.WriteString(th.StatsStyle.Render(" ──────────────────────────────────────────────── "))
	s.WriteString("\n")

	if len(v.commits) == 0 {
		s.WriteString(th.StatsStyle.Render(" No commits yet"))
		s.WriteString("\n")
	} else {
		for _, commit := range v.commits {
			...
			hashStyle := lipgloss.NewStyle().Foreground(th.Info)
			s.WriteString(fmt.Sprintf(" %s %s\n",
				hashStyle.Render(commit.Hash),
				th.StatsStyle.Render(subject)))
		}
	}
