...
func TestBackendOneShotCommandArgv(t *testing.T) {
	// The one-shot model comes from the model table, not from a compiled-in
	// constant, so pin the defaults to keep the expected argv hermetic.
...
// TestClaudeOneShotCommandUsesModelTable pins the merged behaviour: the claude
// one-shot call takes its model from models.json rather than a hardcoded
// "haiku", so changing classifier_model changes commit-message and MR
// generation too — not just the smart router.
func TestClaudeOneShotCommandUsesModelTable(t *testing.T) {
	SetModels(&config.ModelsConfig{
		Backends: map[string]config.BackendModels{
			"claude": {ClassifierModel: "sonnet"},
		},
	})
	t.Cleanup(func() { SetModels(nil) })

	_, args := NewClaudeBackend().OneShotCommand("hello")
	var got string
	for i, a := range args {
